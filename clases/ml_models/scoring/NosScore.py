...
class ScoreDynamic:
    ...
    def __init__(self, year_data: pd.DataFrame, df: pd.DataFrame, dimension_cols: list = None):
        ...
        print(f"ScoreDynamic inicializado para dimensiones: {', '.join(self.dimension_cols)}")
    ...
    def calcular_score(self, fiscal_year=2025):
        ...
        print(f"Calculando componente 1: Rate NOS para {', '.join(self.dimension_cols)}...")
        ...
        print(f"Calculando componente 2: Growth Rate Volume...")
        ...
        print(f"Calculando componente 3: NOS SU Efficiency...")
        ...
        print(f"Calculando componente 4: Growth Rate NOS...")
        ...
        print("Aplicando penalizaciones críticas...")
        # Penalizar casos críticos donde NOS es negativo (situación inaceptable)
        self.df['SCORE'] += np.where(self.df['nos'] < 0, -5.0, 0)
        
        # Penalizar casos críticos donde variation_rate_volume es negativo (decrecimiento)
        self.df['SCORE'] += np.where(self.df['variation_rate_volume'] < 0, -3.0, 0)
        
        # Penalizar casos críticos donde variation_rate_nos es negativo (pérdida de eficiencia)
        self.df['SCORE'] += np.where(self.df['variation_rate_nos'] < 0, -3.0, 0)

        # NORMALIZACIÓN POR RANKING PERCENTIL (distribución gradual)
        print("Aplicando normalización por ranking percentil...")
        ...
        print("Seleccionando columnas finales para reporte...")
